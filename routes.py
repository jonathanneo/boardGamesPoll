...
def viewPoll(poll_id):
    ...
    result = db.session.query(Option, func.count(votes.c.id_option)).outerjoin(votes).group_by(Option.id).filter(Option.id_poll==poll_id).order_by(func.count(votes.c.id_option)).all()
